...
router.post("/admin/approve/:sellerId", async (req, res, next) => {
  try {
    ...
    if (sellerDetails.approvalStatus !== "Pending") {
      return res.status(400).send("Request has already been processed by the admin.");
    }
    ...
    const mail = {
      ...
      message: 'Your product has been approved and is now visible on our platform.',
    };
...
router.post("/admin/reject/:sellerId", async (req, res,next) => {
  try {
    ...
    const mail = {
      ...
      to: sellerDetails.contactInfo.email,
      subject: 'Product Approval Status',
      message: 'Your product has been Rejected, please Visit nearest dnk center for verification.',
    };
    await sendEmail(mail);
    next()
  } catch (error) {
...
router.get('/admin/pending-product-requests', async (req, res) => {
  try {
    ...
    const pendingProductRequests = await product.find({ approvalStatus: 'Pending' });
    ...
    res.status(200).json({ message: 'Pending product requests saved to JSON file.' });
  } catch (error) {
